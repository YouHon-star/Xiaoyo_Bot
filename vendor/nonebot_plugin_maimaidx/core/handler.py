...
# 渲染线程池：PIL 渲染是 CPU 密集且部分持有 GIL，
# 用线程池避免多人同时请求时阻塞事件循环（2核机器建议 2-3 worker）
_RENDER_POOL = ThreadPoolExecutor(max_workers=3, thread_name_prefix="b50-render")
...
async def draw_best50(
    ...
    *,
    ...
    image = await loop.run_in_executor(_RENDER_POOL, lambda: asyncio.run(b50.draw()))
...
async def draw_level_score_list(
    ...
    def _render() -> str:
        ...
        return score.draw_score_list(rating, new_play_result, page, end_page)

    image = await loop.run_in_executor(_RENDER_POOL, _render)
    return MessageSegment.image(image)

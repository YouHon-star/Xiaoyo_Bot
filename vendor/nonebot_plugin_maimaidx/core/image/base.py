...
from .tools import DrawText, open_image_cached, song_chart
...
class ScoreBaseImage(AssetsImage):
    ...
    def whiledraw(self, data: list[PlayedResult], dx: bool = False, list_y: int = 0):
        ...
        for num, info in enumerate(data):
            ...
            cover = open_image_cached(
                str(song_chart(info.song_id)), (75, 75)
            )
            type_ = open_image_cached(str(pic_dir / f"{info.type.upper()}.png"), (37, 14))
            li = min(info.level_index, 4)
            if info.rate.islower():
                rate = open_image_cached(
                    str(
                        pic_dir
                        / self.theme.value
                ...
                rate = open_image_cached(
                    str(pic_dir / self.theme.value / f"UI_TTR_Rank_{info.rate}.png"),
                    (63, 28),
                )

            self._im.alpha_composite(self._diff_bg[li], (x, y))
            self._im.alpha_composite(cover, (x + 12, y + 12))
            self._im.alpha_composite(type_, (x + 51, y + 91))
            self._im.alpha_composite(rate, (x + 92, y + 78))
            if info.fc:
                fc = open_image_cached(
                    str(pic_dir / f"UI_MSS_MBase_Icon_{COMBO_MAP[info.fc]}.png"),
                    (34, 34),
                )
                self._im.alpha_composite(fc, (x + 154, y + 77))
            if info.fs:
                fs = open_image_cached(
                    str(pic_dir / f"UI_MSS_MBase_Icon_{SYNC_MAP[info.fs]}.png"),
                    (34, 34),
                )
            ...
            ds = mai.total_level_value_map.get(f"{info.song_id}-{li}", 0)
            self._tb.draw(
                x + 93,
                y + 65,
                15,

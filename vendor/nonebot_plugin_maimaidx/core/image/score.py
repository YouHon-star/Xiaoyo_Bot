...
from .tools import image_to_base64, open_image_cached, song_chart
...
class DrawScore(ScoreBaseImage):
    ...
    def _while_pic(self, data: list[NotPlayedResult], start_y: int = 200):
        ...
        for num, v in enumerate(data):
            ...
            li = min(v.level_index, 4)
            cover = open_image_cached(str(song_chart(v.song_id)), (55, 55))
            self._im.alpha_composite(cover, (x, y))
            self._im.alpha_composite(self._diff_pg_bg[li], (x - 4, y - 4))
            self._tb.draw(
                x + 36, y + 3, 12, v.song_id, self._diff_text_color[li], "mm"
            )
    ...
    def while_rise_pic(self, data: list[RiseResult], low_score: int, isdx: bool):
        ...
        for index, _d in enumerate(data):
            ...
            li = min(_d.level_index, 4)

            rate = open_image_cached(
                str(self._pic_theme_path / f"UI_TTR_Rank_{RANK_MAP[_d.rate]}.png"),
                (63, 28),
            )

            self._im.alpha_composite(self._rise_bg[li], (x + 30, y))
            self._im.alpha_composite(
                open_image_cached(str(song_chart(_d.song_id)), (80, 80)),
                (x + 55, y + 41),
            )
            self._im.alpha_composite(
                open_image_cached(str(pic_dir / f"{_d.type.upper()}.png"), (60, 22)),
                (x + 240, y + 114),
            )
            if _d.old_rate:
                oldrate = open_image_cached(
                    str(self._pic_theme_path / f"UI_TTR_Rank_{_d.old_rate}.png"),
                    (63, 28),
                )
                self._im.alpha_composite(oldrate, (x + 145, y + 82))
            self._im.alpha_composite(rate, (x + 305, y + 82))

            diff_color = self._diff_text_color[li]
            id_color = self._id_text_color[li]

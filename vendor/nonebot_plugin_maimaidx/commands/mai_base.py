...
async def _(message: Message = Arg("code"), user: User = Depends(GetOrCreateSender)):
...
async def _(
    ...
    if (
        ...
        if _song is not None:
            song = _song
    await mai_what.send("正在绘制中，请稍候…", reply_message=True)
...
async def _(
    ...
    if len(songs) == 0:
        ...
        await random_song.send("正在绘制中，请稍候…", reply_message=True)
...
async def _(match: Match[str] = RegexMatched(), user: User = Depends(GetUserAndAuth)):
    ...
    await rise_score.send("正在绘制中，请稍候…", reply_message=True)
...
async def _(message: Message = CommandArg()):
    ...
    if args.isdigit():
        ...
        name = args.lower()
    await rating_ranking.send("正在绘制中，请稍候…", reply_message=True)

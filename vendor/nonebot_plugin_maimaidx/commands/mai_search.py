...
async def _(
    ...
    if len(songs) == 1:
        await search.send("正在绘制中，请稍候…", reply_message=True)
...
async def _(
    ...
    if alias_data:
        if len(alias_data) != 1:
            ...
            if song:
                await search_alias_song.send("正在绘制中，请稍候…", reply_message=True)
...
async def _(
    ...
    if not song:
        ...
        await query_chart.send("正在绘制中，请稍候…", reply_message=True)

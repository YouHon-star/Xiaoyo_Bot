...
async def _(match: Match[str] = RegexMatched()):
    ...
    if rating in LEVEL_LIST[:6]:
        ...
        if rating == "15":
            ...
            await rating_table.send("正在绘制中，请稍候…", reply_message=True)
...
async def _(match: Match[str] = RegexMatched(), user: User = Depends(GetUserAndAuth)):
    ...
    if ra in LEVEL_LIST[:6]:
        ...
        await rating_table_pfm.send("正在绘制中，请稍候…", reply_message=True)
...
async def _(
    ...
    await matcher.send("正在绘制中，请稍候…", reply_message=True)
...
async def _(match: Match[str] = RegexMatched(), user: User = Depends(GetUserAndAuth)):
    ...
    await level_progress.send("正在绘制中，请稍候…", reply_message=True)
...
async def _(match: Match[str] = RegexMatched(), user: User = Depends(GetUserAndAuth)):
    ...
    await level_score_list.send("正在绘制中，请稍候…", reply_message=True)
